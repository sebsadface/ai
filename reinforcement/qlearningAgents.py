...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        "*** YOUR CODE HERE ***"
        self.qValues = util.Counter()  # This initializes the Q-values
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        q_value = sum(self.weights[f] * value for f, value in features.items())
        return q_value
    ...
    def update(self, state, action, nextState, reward):
        ...
        maxQ_next = max(self.getQValue(nextState, a) for a in nextActions) if nextActions else 0
        currentQ = self.getQValue(state, action)
        difference = (reward + self.discount * maxQ_next) - currentQ
        features = self.featExtractor.getFeatures(state, action)
