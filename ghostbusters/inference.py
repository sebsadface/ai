...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[PAC] = variableDomainsDict[GHOST0] = variableDomainsDict[GHOST1] = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]

    variableDomainsDict[OBS0] = variableDomainsDict[OBS1] = list(range(X_RANGE + Y_RANGE - 2 + MAX_NOISE + 1))
    "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        ws = DiscreteDistribution()
        for part in self.particles:
            ws[part] += self.getObservationProb(observation, pacPos, part, jailPos)
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newParts.append(self.getPositionDistribution(gameState, oldPos).sample())
