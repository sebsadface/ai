...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        mostLikelyPos = [
            ghostDist.argMax() for ghostDist in livingGhostPositionDistributions
        ]
        ...
        bestAct = None
        minActDist = float('inf')
        for act in legal:
            nxtPos = Actions.getSuccessor(pacmanPosition, act)
            dist = self.distancer.getDistance(nxtPos, closestGhost)
            if dist < minActDist:
                minActDist = dist
                bestAct = act
